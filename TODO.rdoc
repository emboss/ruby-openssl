== OpenSSL::ASN1::Template

=== asn1_declare :ANY

Provide a generic ANY type - asn1_any can then be discarded.

=== asn1_sequence_of, asn1_set_of

Currently the allowed sizes are generally (0..inf). It
should also be possible to restrict this to e.g. (4..6) etc.

=== asn1_sequence_of_choice, asn1_set_of_choice

Could be provided by creating a temporary choice type that is attached to
the ordinary asn1_sequence_of and asn1_set_of. the *of_choice methods might
come in handy in situations where the namespace should not be polluted by 
being forced to create a template for an otherwise useless ASN.1 construct.

=== Set order

* Take care of SET and SET OF ordering

=== Tests 

* Test for infinite length explicitly tagged primitive with single value
( not constructed)
* Test for choice with inner template type

=== String encoders

Introduce encoders that force a certain encoding that is specific to the string type?

=== Threads
Tested threaded encoding for SET_OF. Unfortunately much slower than sequential.
Experiment with thread pool and pool sizes. Same result.